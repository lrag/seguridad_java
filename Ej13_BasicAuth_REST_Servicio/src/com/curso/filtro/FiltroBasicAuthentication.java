...
public class FiltroBasicAuthentication implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
		...
		if(auth == null) {
			...
			rp.getOutputStream().println("Falta la cabecera de autorización");
			return;
		}
		...
		try {
			...
			rp.getOutputStream().println("Cabecera de autorización mal construida");
			return;
		} 
		...
		if(usr == null || !usr.getPw().equals(pw) ) {
			rp.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
			rp.getOutputStream().println("Credenciales incorrectas");			
			return;
		}
